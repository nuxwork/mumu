 ...
 * 
 */
public class SlideTabPager extends LinearLayout implements ViewPager.OnPageChangeListener {
	...
	protected void onFinishInflate() {
		...
		mTabWidget = (TabWidget) findViewById(chat.mumu.lego.R.id.tabs);
		mViewPager = (ViewPager) findViewById(chat.mumu.lego.R.id.pagers);

		if (mTabWidget == null || mViewPager == null) {
			throw new RuntimeException("Your content must have a TabWidget whose id attribute is "
					+ "'R.id.tabs' and a ViewPager whose id attribute is " + "'R.id.pagers'");
		}
	...
	private void handTabListener() {
		...
		try {
			...
			Method method = cls.getDeclaredMethod("setTabSelectionListener", listener);

			Object listenerImpl = Proxy.newProxyInstance(listener.getClassLoader(),
					new Class[] { listener }, new OnTabSelectionChangedImpl());
	...
	private class OnTabSelectionChangedImpl implements InvocationHandler {
		...
		public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
			...
			if ("onTabSelectionChanged".equals(method.getName()) && args != null
					&& args.length == 2) {
